...
class Util {
     ...
     * (array) extensions - in lower case, (array) ignore, (string) prefix, (string) filename]
    ...
    public static function uploadFiles(\Object $data, array $options = array()) {
        ...
        foreach ($data->toArray(TRUE) as $ppt => $info) {
            ...
            foreach ($info['name'] as $key => $name) {
                ...
                $tmpName = $info['tmpName'] ? $info['tmpName'][$key] : $info['tmp_name'][$key];
                $pInfo = pathinfo($name);
                ...
                $nm = ($options['filename']) ? $options['filename'] : basename($pInfo['filename']);
                $savePath = $dir . $options['prefix'] . preg_replace('/[^A-Z0-9._-]/i', '_', $nm) . '.' . $pInfo['extension'];
     ...
     * @param bool $returnArray TRUE to return the found array, FALSE to return the key and 
     * ignores param multiple
     * @return array|null The array containing the expected value
     */
    public static function searchArray(array $array, $value, $recursive = false, $key = array(), $multiple = false, $returnArray = true) {
        if (!is_array($value))
            $value = array($value);
        ...
        foreach ($array as $ky => $val) {
            if (is_array($val)) {
                ...
                if (count($key)) {
                    ...
                    if ($fnd) {
                        $found = $returnArray ? $val : $ky;
                        break;
                    }
                }
                $ret = static::searchArray($val, $value, $recursive, $key, $multiple, $returnArray);
                if (count($ret)) {
                    if ($multiple)
                        $found = array_merge($found, $ret);
                    else
                        return $ret;
                } else if (!is_array($ret) && !is_null($ret)) {
                    return $ret;
                }
                continue;
            }
            ...
            if ($val === $value[$keys[$ky]]) {
                if ($returnArray && $multiple) {
                    $k = ($multiple === true) ? 0 : $array[$multiple];
                    $found[$k] = $array;
                } else
                    return $returnArray ? $array : $ky;
            }
        }
